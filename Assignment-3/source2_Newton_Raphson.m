...
%     thetha_init=[0;0];
%     for n=1:1:N
%         yR_n=recieverPower(n,M,sigma,radius);
%         thetha_opt=newtonRaphson(n,M,Rpos,eps,yR_n,beta,beta_sq,p);
%         thetha_sum=thetha_sum+thetha_opt;
%     end
%     thetha_final=(1/N)*(thetha_sum);
%     meansqerr=norm(thetha_final-s2_pos_gd);
...
% xlabel('N')
% ylabel('MSE')
% saveas(gcf,'S2_res_N_vs_MSE.png')
...
set(f1, 'Visible', 'off');
title('mse vs snr ')
plot(snr_db,meanErrorList)
xlabel('SNR(db)')
ylabel('MSE')
saveas(gcf,'S2_res_SNR_vs_MSE.png')
...
% f function 
function fval = ft(n,p,r,l)
    fval=10*log10(s2(n,p)./g(r,l)); 
end 
...
function yR=recieverPower(n,M,sigma,radius)
    w=sigma*randn([1 M]);
    dterm=radius.^2/2*randn([1 M])+0.25*radius.^2;
    p=1;
    for i=1:1:M
        yR(i)=10*log10(p*n*1/dterm(i))+w(i)*(sigma)*rand(1);
    end
    yR=transpose(yR);
end 

% d(lnP)/dx function n=1
function val = dlnpx(n,r,y,l,factor,p)
    rx=r(1);
    lx=l(1);
    num= (y-ft(n,p,r,l))*(rx-lx);
    den=g(r,l);
    val=factor*(num./den);
end 

% d(lnP)/dy function for s1(n) signal . 
function val = dlnpy(n,r,y,l,factor,p)
    ry=r(2);
    ly=l(2);
    num=(y-ft(n,p,r,l))*(ry-ly);
    den=g(r,l);
    val=factor*(num./den);
end 

% @S1First derivative of log-likelihood function [ d(ln(p)/dx d(ln(p)/dy ]
function p1=lnpvec(n,M,R,Y,l,factor,p)
    fx=0;
    fy=0;
    for i=1:1:M
        r_i=R(:,i);
        y_i=Y(i);
        fx=fx+dlnpx(n,r_i,y_i,l,factor,p);
        fy=fy+dlnpy(n,r_i,y_i,l,factor,p);
    end 
    p1=[fx;fy];
end 
...
function thetha_opt=newtonRaphson(n,M,R,eps,Y,beta,beta_sq,p)
    thetha_old=randn([2 1]);
    factor1=beta;
    factor2=beta_sq;
    err=inf;
    count=0;
    while(err>=eps)
        Q=mtimes(I(M,R,thetha_old,factor1),lnpvec(n,M,R,Y,thetha_old,factor2,p));
        if(isinf(Q)|isnan(Q))
